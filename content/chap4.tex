...
Thus speed-up is approximately $1/(1-\rho_{XY}^2)$. The sign of $\rho_{XY}$ is absorbed by $b^*$; only $|\rho_{XY}|$ matters.
...
Antithetic variates pair inputs to create negative dependence while preserving the marginal law. If $U\sim\operatorname{Unif}[0,1]$, use $U$ and $1-U$; for a symmetric normal input, use $Z$ and $-Z$.
...
$\frac{\hat{Y}_{AV}-\mathbb{E}[Y]}{\sigma_{AV}/\sqrt{n}}\Rightarrow N(0,1),\qquad \sigma_{AV}^2=\operatorname{var}\left(\frac{Y_i+\tilde{Y}_i}{2}\right)$.

A CI is $\hat{Y}_{AV}\pm z_{\delta/2}s_{AV}/\sqrt{n}$, where $s_{AV}$ is the sample sd of pair averages.

\subsubsection*{When it helps}
...
$\operatorname{var}[\hat{Y}]=\sum_{i=1}^Kp_i^2\frac{\sigma_i^2}{n_i}=\frac{\sigma^2(q)}{n},\qquad \sigma^2(q)=\sum_{i=1}^K\frac{p_i^2}{q_i}\sigma_i^2$.
...
$q_i^*=\frac{p_i\sigma_i}{\sum_{\ell=1}^Kp_\ell\sigma_\ell},\qquad \sigma^2(q^*)=\left(\sum_{i=1}^Kp_i\sigma_i\right)^2$.
...
$f_\theta(x)=\frac{e^{\theta x}f(x)}{\mathbb{E}[e^{\theta X}]},\qquad \theta\in\mathbb{R}$.
