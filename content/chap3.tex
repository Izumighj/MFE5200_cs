...
$W(t)-W(s)\sim N(0,t-s),\qquad 0\le s<t\le T$.
...
$A_{ij}=\sqrt{t_j-t_{j-1}}\,\mathbf{1}_{\{i\ge j\}},\qquad t_0=0$.

This Cholesky method is the matrix form of the random-walk construction. Direct multiplication is $O(n^2)$, while the recursive construction is $O(n)$.

For $X\sim\operatorname{BM}(\mu,\sigma^2)$, the mean vector has entries $\mu t_i$, the covariance matrix is $\sigma^2C$, and the factor is $\sigma A$.
...
This may require a separate factorization for each time step.
...
where $\Delta t_i=t_{i+1}-t_i$. This is exact at the simulated dates and has no model discretization error.
...
$\mathbb{E}[S(t)]=e^{\mu t}S(0),\qquad \operatorname{var}[S(t)]=e^{2\mu t}S^2(0)(e^{\sigma^2t}-1)$.
...
$(\max\{c_1S_1(T),c_2S_2(T),\ldots,c_dS_d(T)\}-K)^+$.

Two-asset barrier example:

$\mathbf{1}_{\{\min_{i=1,\ldots,n}S_2(t_i)<b\}}(K-S_1(T))^+$.
